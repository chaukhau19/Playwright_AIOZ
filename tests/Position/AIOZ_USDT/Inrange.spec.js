...
test("Create position In range 0.01%", async ({ wallet, page }) => {
  ...
  await page.waitForTimeout(1000);
  await page.getByTestId('pool-nav-link').click();
  await page.getByRole('link', { name: '+ New position' }).click();
  await page.getByRole('button', { name: 'Select a token' }).click();
  await page.getByText('Tether USD').click();
  await page.getByRole('button', { name: 'I understand' }).click();
  await page.getByRole('button', { name: '% Best for very stable pairs.' }).click();

  await functionPage.Total_Token_Before_Swap();
  
  await page.locator('#add-liquidity-input-tokena').getByPlaceholder('0').fill('1');
  ...
  await functionPage.Token_Confirm_Swap();
  
  await functionPage.compare_Token();
