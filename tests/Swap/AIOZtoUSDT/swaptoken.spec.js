...
test("Swap AIOZ to USDT", async ({ wallet, page }) => {
  ...
  // while (retries > 0) {
  //   try {
  //     await page.goto("https://aiozswap-web.vercel.app/#/swap", { waitUntil: "domcontentloaded", timeout: 60000 });

  //     // Kiểm tra nếu vẫn bị about:blank, reload lại
  //     if (page.url() === "about:blank") {
  //       console.log("⚠️ Page still blank, reloading...");
  //       await page.reload();
  //       await page.waitForTimeout(5000);
  //     }
  //     break;
  //   } catch (error) {
  //     console.log(`⚠️ Page load error, retrying (${4 - retries}/3)`);
  //     retries--;
...
for (let i = 0; i < maxRetries; i++) {
    try {
        ...
        console.error(`⚠️ Lần thử ${i + 1} thất bại: ${error.message}`);
        if (i === maxRetries - 1) throw error; // Nếu thử 3 lần vẫn lỗi thì dừng
    }
}
  ...
  await page.getByText('Tether USD').click();
  ...
  await functionPage.PointsBefore_ConfirmSwap();
  
  await functionPage.comparePoints();
